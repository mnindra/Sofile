...
<script>
    window.activePage = "job_application_menu";

    let load_data = () => {
        $.get("<?= SITE_URL . '?page=admin/JobApplication&action=all' ?>", (data) => {
            $("#table-body").empty();
    ...
    let destroy = (id) => {
        $.get("<?= SITE_URL . '?page=admin/JobApplication&action=destroy&id=' ?>" + id, (data) => {
            load_data();
        });
    };
